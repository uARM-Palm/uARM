...
static void socDmaPrvChannelDescrFetch(struct SocDma* dma, struct PxaDmaChannel *ch)	//you must call socDmaPrvChannelIrqRecalc() after this func 
{
	...
	//fprintf(stderr, "DESCR fetched from 0x%08x: {0x%08x 0x%08x 0x%08x 0x%08x}\n", dar, nextD, nextS, nextT, nextC);
}

static bool socDmaPrvChannelCheckForEnd(struct SocDma* dma, uint_fast8_t channel)	//return true if irq need updating after what we did
{
...
static bool socDmaPrvChannelDoBurst(struct SocDma* dma, uint_fast8_t channel)		//return true if irq need updating after what we did
{
	struct PxaDmaChannel *ch = &dma->channels[channel];
	...
	while (num--) {
		...
		if (!memAccess(dma->mem, src, each, false, &t) || !memAccess(dma->mem, dst, each, true, &t)) {
...
static void socDmaPrvChannelActIfNeeded(struct SocDma* dma, uint_fast8_t channel)
{
	...
	//check for end
	if (socDmaPrvChannelCheckForEnd(dma, channel))
		irqUpdate = true;
	...
	if (socDmaPrvChannelRunning(dma, ch)) {			//if we are running, get working...
		...
		if (doWork) {
			...
			do {
				...
				if (socDmaPrvChannelDoBurst(dma, channel))
					irqUpdate = true;
...
static void socDmaPrvChannelMaybeStart(struct SocDma* dma, struct PxaDmaChannel *ch, uint32_t prevCsrVal)
{
...
static bool socDmaPrvChannelRegWrite(struct SocDma* dma, uint_fast8_t channel, uint_fast8_t reg, uint32_t val)
{
	struct PxaDmaChannel *ch = &dma->channels[channel];
	...
	if (reg == REG_DAR) {
		...
		if (val && !(ch->CSR & 0x40000000ul))
			maybeStart = true;
	}
	else if (reg == REG_SAR) {
		if (ch->CSR & 0x40000000ul)
			checkForStart = true;
		ch->dsAddrWriten = 1;
		ch->SAR = val;
	}
...
static bool socDmaPrvChannelRegRead(struct SocDma* dma, uint_fast8_t channel, uint_fast8_t reg, uint32_t *retP)
{
	switch (reg) {
		case REG_DAR:
...
static bool socDmaPrvMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
	...
	if (write) {
		...
		switch(pa >> 6) {		//weird, but quick way to avoid repeated if-then-elses. this is faster
			case 0:
				...
				reg = REG_CSR;
				set = pa;
				if (!socDmaPrvChannelRegWrite(dma, set, reg, val))
					return false;
				break;
		...
		switch(pa >> 6) {		//weird, but quick way to avoide repeated if-then-elses. this is faster
			case 0:
				...
				reg = REG_CSR;
				set = pa;
				if (!socDmaPrvChannelRegRead(dma, set, reg, &val))
					return false;
				break;
