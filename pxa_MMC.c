...
static bool pxaMmcPrvDataFifoW(struct PxaMmc *mmc, uint32_t val)
{
	...
	if (mmc->fifoBytes == mmc->blkLen) {
		...
		switch (ret) {
			case SdDataErrWrongBlockSize:		//would manifest as a crc error
			case SdDataErrWrongCurrentState:	//would manifest as a timeout but we report all as crc errors
			case SdDataErrBackingStore:
			default:
...
static bool mmcSendCommand(struct PxaMmc *mmc)
{
	...
	if (success && len && doCrcCheck)
		crcFail = vsdCRC7(reply, len) != reply[len];
	...
	// data to handle?
	if (success && (mmc->cmdat & 4))
		pxaMmcPrvDataXferStart(mmc);
	
	//all cases except write, we're not busy by response time
	if (!success || !(mmc->cmdat & 4) || !(mmc->cmdat & 8))
		mmc->stat |= 0x1000;	//not busy
...
static bool pxaMmcPrvMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
	...
	//if (write)
	//	fprintf(stderr, "MMC 0x%08x->[0x%08x]\n", *(uint32_t*)buf, (unsigned)(PXA_MMC_BASE + 4 * pa));
	...
	//				fprintf(stderr, "MMC clock set to %u khz\n", 20000 >> val);
		...
		case 15:	//rep buf
			if (write)
				...
				val = mmc->respBuf[0];
				memmove(mmc->respBuf, mmc->respBuf + 1, sizeof(mmc->respBuf) - sizeof(*mmc->respBuf));
			}
			break;
	...
	//	if (!write)
	//		fprintf(stderr, "MMC [0x%08x] -> 0x%08x\n", (unsigned)(PXA_MMC_BASE + 4 * pa), val);
	}
...
void pxaMmcInsert(struct PxaMmc *mmc, struct VSD* vsd)
{
	mmc->vsd = vsd;
}
