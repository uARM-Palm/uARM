...
static bool pxa270UdcPrvMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
	...
	if (pa >= 0x100 / 4 && pa < 0x160 / 4) {
		...
		case 0x000 / 4:		//UDCCR
			if(write)
				udc->udccr = ((udc->udccr & 0x70011ffeul) | (val & 0x80000001ul)) &~ (val & 0x0000001cul);
			else
				val = udc->udccr;
			break;
