...
struct PxaKpc {
	...
	//stimuli
	uint64_t prevKeys;
	uint8_t matrixKeys[8];	//array indexed by column[output]. bits indexed by row (input). 1 = pressed
	uint8_t directKeys;		//bitfield, 1 = down
	uint8_t numMatrixKeysPressed;
	uint16_t jogSta[2];		//same format as KPREC reg, except we use bits 12..13 for jog sensor status
};
...
static void pxaKpcPrvDoAutoScan(struct PxaKpc *kpc, bool causeIrq)
{
	...
	if (causeIrq && nowKeys != kpc->prevKeys)
		kpc->kpc |= 0x00400000ul;
...
static void pxaKpcPrvMatrixRecalc(struct PxaKpc *kpc, bool lastKeyChangeWasDown)
{
	...
	if (kpc->kpc & 0x20000000ul) {
		...
		if ((kpc->kpc & 0x00200000ul) && ((lastKeyChangeWasDown && kpc->numMatrixKeysPressed != 1) || (!lastKeyChangeWasDown && kpc->numMatrixKeysPressed != 0)))
			return;
...
static bool pxaKpcPrvMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
...
void pxaKpcMatrixKeyChange(struct PxaKpc *kpc, uint_fast8_t row, uint_fast8_t col, bool isDown)
{
	...
	if (row >= 8)
		ERR("only eight matrix key rows exist\n");
	
	if (col >= 8)
		ERR("only eight matrix key cols exist\n");
...
void pxaKpcDirectKeyChange(struct PxaKpc *kpc, uint_fast8_t keyIdx, bool isDown)
{
	...
	if (keyIdx >= 8)
		ERR("only eight direct keys exist\n");
	
	if (!(kpc->kpc & 2))
		fprintf(stderr, "setting direct keys when direct keypad is disabled\n");
...
void pxaKpcJogInput(struct PxaKpc *kpc, uint_fast8_t jogIdx, bool up)
{
	...
	if (up) {
		...
		//rotate it
		kpc->jogSta[jogIdx] = (kpc->jogSta[jogIdx] & 0xc0ff) | ((uint32_t)jogNext[(kpc->jogSta[jogIdx] >> 12) & 3]) << 12;
	}
	else {
