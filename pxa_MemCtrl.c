...
static bool pxaMemCtrlrPrvClockMgrMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
...
struct PxaMemCtrlr* pxaMemCtrlrInit(struct ArmMem *physMem, uint_fast8_t socRev)
{
	...
	for (i = 0; i < 3; i++)
		mc->msc[i] = 0x7ff07ff0ul;
	
	mc->arbCntrl = 0x00800234ul;
	for (i = 0; i < 4; i++)
		mc->bscntr[i] = 0x55555555ul;
	mc->lcdbscntr = 0x05;
	
	if (!memRegionAdd(physMem, PXA_MEM_CONTROLLER_BASE, PXA_MEM_CONTROLLER_SIZE, pxaMemCtrlrPrvClockMgrMemAccessF, mc))
