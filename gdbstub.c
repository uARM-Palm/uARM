...
struct stub {
	...
	static void gdbStubPrvSendAck(struct stub *stub)
	{
	...
	static void gdbStubPrvGetPacket(struct stub *stub)
	{
		...
		do {
			...
			if (ret == 0)
				ERR("debugger connection was closed\n");
			if (ret == -1) {
				if (errno == EINTR)
					continue;
				else
	...
	static bool gdbStubPrvWpOp(struct stub *stub, bool set, uint32_t addr, uint_fast8_t size, bool forWrite, bool forRead)
	{
		...
		if (set) {
			...
			for (i = 0; i < stub->numWp; i++) {
				...
				if (stub->wp[i].addr == addr && stub->wp[i].size == size && !stub->wp[i].write == !forWrite && !stub->wp[i].read == !forRead) {
	...
	static bool gdbStubPrvBpWpOp(struct stub *stub, bool set, char type, uint32_t addr, uint32_t len)
	{
		//breakpoint?
		if (type == '0' || type == '1')
			return gdbStubPrvBpOp(stub, set, addr);
	...
	static uint32_t gdbStubPrvMemWrite(struct stub *stub, char *data, uint32_t addr, uint32_t len)
	{
		...
		if (addr & 1) {
			...
			if (!cpuMemOpExternal(stub->cpu, data, addr, 1, true))
				return data - orig;
			data++;
			len--;
			addr++;
		...
		if ((addr & 2) && len >= 2) {
			...
			if (!cpuMemOpExternal(stub->cpu, data, addr, 2, true))
				return data - orig;
			data += 2;
			len -= 2;
			addr += 2;
		...
		while (len >= 4) {
			...
			if (!cpuMemOpExternal(stub->cpu, data, addr, 4, true))
				return data - orig;
			data += 4;
			len -= 4;
			addr += 4;
	...
	static bool gdbStubPrvMemRead(struct stub *stub, char *out, uint32_t addr, uint32_t len)
	{
		...
		if (len > (stub->pktBufSz - 1) / 2)
			len = (stub->pktBufSz - 1) / 2;
		
		//use largest possible aligned size to do the accesses
		if (addr & 1) {
			
			if (!cpuMemOpExternal(stub->cpu, vals, addr, 1, false))
				return out - orig;
		...
		if ((addr & 2) && len >= 2) {
			...
			if (!cpuMemOpExternal(stub->cpu, vals, addr, 2, false))
				return out - orig;
		...
		while (len >= 4) {
			...
			if (!cpuMemOpExternal(stub->cpu, vals, addr, 4, false))
				return out - orig;
	...
	static bool gdbStubPrvInterpPacket(struct stub *stub)	//return true if we prepared a reply
	{
		...
		if (in == strstr(in, "qSupported"))
			...
			out[0] = 0;
			if (!gdbStubPrvAddRegToStr(stub, out, regNo))
				goto cmderr;
		}
			...
			if (regNo < 15) {
				...
				cpuSetReg(stub->cpu, 15, (val &~ 1) | ((cpuGetRegExternal(stub->cpu, ARM_REG_NUM_CPSR) & ARM_SR_T) ? 1 : 0));
			}
			else if (regNo == 0x19) {
			...
			if (!gdbStubPrvMemRead(stub, out, addr, len))
				strcpy(out, "00");		//le sigh...gdb does not handle actual read failures, so we report all unreadable memory as zeroes...
		}
	...
	static void gdbStubPrvGetAndHandleCommands(struct stub *stub)
	{
		while (stub->runState == RunStateStopped) {
			...
			if (gdbStubPrvInterpPacket(stub))
				gdbStubPrvSendPacket(stub, stub->pktBuf, true);
		}
	}
	...
	static int gdbStubPrvCheckWatchpoints(struct stub *stub, uint32_t addr, uint_fast8_t sz, bool write)
	{
		...
		for (i = 0; i < stub->numWp; i++, wp++) {
			...
			if (((wp->addr > addr) ? wp->addr : addr) >= ((wp->addr + wp->size < addr + sz) ? wp->addr + wp->size : addr + sz))
				continue;
	...
	static bool gdbStubPrvCheckInterrupt(struct stub *stub)
	{
		...
		ret = recv(stub->sock, &c, 1, 0);
		if (ret != 1)
			ERR("failed to receive one byte: %d %d\n", ret, errno);
		
		if (c != 3)
			ERR("RXed byte ('%c' 0x%02x) is not interrupt when only that is possible\n", c, c);
...
void gdbStubReportPc(struct stub *stub, uint32_t pc, bool thumb)
{
	if (stub->sock < 0)
		...
		if (stub->runState == RunStateSingleStep) {
			...
			strcpy(stub->stopReason, "S02");		//Ctrl+C sends "INT" which is 2 (we also seem to be able to send STOP which is 0x11)
			stub->runState = RunStateStopped;
		}
		else if (stub->runState == RunstateRunning && gdbStubPrvCheckBreakpoints(stub, pc, thumb) >= 0) {
			
			strcpy(stub->stopReason, "S05");		//breakpoint send "TRAP" which is 5
			stub->runState = RunStateStopped;
		}
		
		if (stub->runState == RunStateStopped)
...
void gdbStubReportMemAccess(struct stub *stub, uint32_t addr, uint_fast8_t sz, bool write)
{
	...
	if (stub->sock < 0)
	...
	#ifdef GDB_STUB_ENABLED
		if (stub->runState != RunstateRunning)
			return;
		...
		//gdb expects the insruction to be aborted. it will then expect to remove the watchpoint, and step
		//over it. this means that we need to:
...
struct stub* gdbStubInit(struct ArmCpu *cpu, int port)
{
	...
	if (port < 0)
		stub->sock = -1;
			...
			if (sock == -1)
				ERR("gdb socket creation fails: %d", errno);
			...
			if (ret)
				ERR("gdb socket bind fails: %d", errno);
			...
			if (ret)
				ERR("gdb socket listen fails: %d", errno);
			
			do {
				ret = accept(sock, (struct sockaddr*)&sa, &sl);
			} while (ret == -1 && errno == EINTR);
			if (ret == -1)
				ERR("gdb socket accept fails: %d", errno);
			close(sock);
			stub->sock = ret;
