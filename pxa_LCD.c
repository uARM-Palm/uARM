...
static bool pxaLcdPrvMemAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* buf)
{
...
static uint32_t pxaLcdPrvGetWord(struct PxaLcd *lcd, uint32_t addr)
{
	...
	if (!memAccess(lcd->mem, addr, 4, false, &v))
		return 0;
	...
	while (len--) {
		...
		switch ((lcd->lccr3 >> 24) & 7) {
			...
			case 4:		//16BPP
				...
				for (i = 0; i < 4; i += 2)
					pxaLcdPrvScreenDataPixel(lcd, data + i);
				break;
		...
		if (lcd->lccr0 & 0x400) {		//got disabled
			...
			case LCD_STATE_DMA_0_START:
				...
				if (lcd->ldcmd[0] & 0x04000000UL) {	//pallette data
					...
					lcd->frameNum++;
					if (!(lcd->frameNum & 63))
						pxaLcdPrvScreenDataDma(lcd, lcd->fsadr[0], len);
				}
