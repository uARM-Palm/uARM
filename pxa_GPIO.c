...
{
	uint32_t val, bit, newVal, oldVal = gpio->levels[which], t;
	uint_fast8_t i;
	...
	while (t) {
		...
		if (gpio->notifF[num])
			gpio->notifF[num](gpio->notifD[num], num, !!(oldVal & (1 << bit)), !!(newVal & (1 << bit)));
	}
...
{	
	socIcInt(gpio->ic, PXA_I_GPIO_all, gpio->detStatus[3] || gpio->detStatus[2] || gpio->detStatus[1] || (gpio->detStatus[0] &~ 3));
