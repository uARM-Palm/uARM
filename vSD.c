...
static uint32_t vsdCalcR1resp(struct VSD *vsd, uint32_t r1bicBits, uint32_t r1orrBits)
{
	...
	if (vsd->acmdShift || vsd->reportAcmdNext)
		r1 |= 1 << 5;
	vsd->reportAcmdNext = 0;
...
static enum SdReplyType vsdPrepareR1resp(struct VSD *vsd, uint8_t cmd, uint8_t *reply, uint32_t r1bicBits, uint32_t r1orrBits)
{
...
static void vsdGetCsdV1(struct VSD *vsd, uint8_t *reply)
{
	...
	reply[6] = (cSize >> 10) | 0x80;					//READ_BL_PARTIAL(1), WRITE_BLK_MISALIGN, READ_BLK_MISALIGN, DSR_IMP, RESERVED, C_SIZE[10..11]
	reply[7] = cSize >> 2;								//C_SIZE[2..9]
	reply[8] = (cSize << 6)	| 0x2D;						//C_SIZE[0..1], VDD_R_CURR_MIN (35mA), VDD_R_CURR_MAX(45mA)
	reply[9] = (cSizeMult >> 1)	| 0xD8;					//VDD_W_CURR_MIN (60mA), VDD_W_CURR_MAX(80mA), C_SIZE_MULT[1..2]
	reply[10] = (sectorSize >> 1) + (cSizeMult << 7);	//C_SIZE_MULT[0], ERASE_BLK_EN, SECTOR_SIZE[1..7]
	...
	reply[14] = 0x00;									//FILE_FORMAT_GRP, COPY, PERM_WRITE_PROTECT, TMP_WRITE_PROTECT, FILE_FORMAT, RESERVED
}
...
static void vsdGetCsdV2(struct VSD *vsd, uint8_t *reply)
{
	...
	reply[6] = 0x00;					//READ_BL_PARTIAL, WRITE_BLK_MISALIGN, READ_BLK_MISALIGN, DSR_IMP, RESERVED[2..5]
	...
	reply[11] = sectorSize << 7;		//SECTOR_SIZE[0], WP_GRP_SIZE
	reply[12] = (writeBlLen >> 2) | 8;	//WP_GRP_ENABLE, RESERVED, R2W_FACTOR(2), WRITE_BL_LEN[2..3]
...
enum SdReplyType vsdCommand(struct VSD *vsd, uint8_t cmd, uint32_t param, void *replyOut /* should be big enough for any reply */)
{
	...
	send_cid:
		...
		case 7:
			if (vsd->state == StateStby && vsd->rca == (param >> 16)) {		//were we addressed
				...
				if (replTyp == SdReply48bits)
					replTyp = SdReply48bitsAndBusy;
			}
			else if ((vsd->state == StateStby || vsd->state == StateTran || vsd->state == StateData || vsd->state == StatePrg) && vsd->rca != (param >> 16)) {		//we were not addressed
		...
		case 13:
			if (wasAcmd)
				//XXX: ACMD13 exists
				replTyp = vsdRejectCmd(vsd, cmd, param, reply, InvalidInCurrentState, wasAcmd);
			else if (vsd->state != StateStby && vsd->state != StateTran && vsd->state != StateData && vsd->state != StateRcv && vsd->state != StatePrg && vsd->state != StateDis)
				replTyp = vsdRejectCmd(vsd, cmd, param, reply, InvalidInCurrentState, wasAcmd);
			else
				replTyp = vsdPrepareR1resp(vsd, cmd, reply, 0, 0);
			break;
		...
		case 18:
			...
			else if (!vsd->hcCard && (param % 512)) {	//param must be a multiple of 512b for non-HC card
