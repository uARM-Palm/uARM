...
bool pxa255dspPrvAccess(struct ArmCpu* cpu, void* userData, bool MRRC, uint8_t op, uint8_t RdLo, uint8_t RdHi, uint8_t acc)
{
	...
	if (MRRC) {	//MRA: read acc0
		...
		dsp->acc0 = (((uint64_t)(cpuGetRegExternal(cpu, RdHi) & 0xff)) << 32) | cpuGetRegExternal(cpu, RdLo);
	}
	
	return true;	
}

bool pxa255dspPrvOp(struct ArmCpu* cpu, void* userData, bool two/* MCR2/MRC2 ? */, bool MRC, uint8_t op1, uint8_t Rs, uint8_t opcode_3, uint8_t Rm, uint8_t acc)
{
	struct Pxa255dsp *dsp = (struct Pxa255dsp*)userData;
	uint64_t addend = dsp->acc0;
	uint32_t Vs, Vm;
	
	if (op1 != 1 || two || MRC || acc != 0)
		return false;		//bad encoding
